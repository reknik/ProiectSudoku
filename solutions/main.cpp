...
{   int i,j;
    f>>n;
    for(i=1;i<=n;i++) //se citesc cele n litere la dispozitie
        f>>c[i];
    for(i=1; i<=n; i++) //se citeste tabela initiala
        for(j=1; j<=n; j++)
            f>>t[i][j];
}
...
{
    int i,j;
    for(i=1; i<=n; i++) //se verifica daca o litera asemenea este gasita pe linia spatiului sau coloana spatiului
